...
function materialRun(value: unknown): value is MaterialProcessingRunView {
  ...
  if (
    typeof item.material_id !== "string" ||
    !uuid.test(item.material_id) ||
    typeof item.source_artifact_id !== "string" ||
    !uuid.test(item.source_artifact_id)
  )
    return false;
...
function sourceView(value: unknown): value is SourceView {
  ...
  return (
    !!item &&
    [item.source_id, item.normalization_id, item.original_artifact_id].every(
    ...
    (item.normalized_artifact_id === null ||
      (typeof item.normalized_artifact_id === "string" &&
        uuid.test(item.normalized_artifact_id))) &&
...
function libraryItem(value: unknown): value is MaterialLibraryItem {
  ...
  if (
    !item.study_sessions.every((value) => {
      ...
      return (
        !!study &&
        typeof study.study_session_id === "string" &&
        uuid.test(study.study_session_id) &&
        revision(study.knowledge_structure_revision, "knowledge-structure") &&
        typeof study.run_id === "string" &&
        uuid.test(study.run_id) &&
        ...
        !claim.evidence.every((value) => {
          ...
          return (
            ...
            (evidence.source_id === undefined ||
              (typeof evidence.source_id === "string" && uuid.test(evidence.source_id))) &&
...
function studySession(value: unknown): value is StudySessionView {
  ...
  return (
    ...
    typeof item.study_session_id === "string" &&
    uuid.test(item.study_session_id) &&
    typeof item.material_id === "string" &&
    uuid.test(item.material_id) &&
    revision(item.knowledge_structure_revision, "knowledge-structure") &&
...
function feedback(value: unknown): value is AnswerFeedbackView {
  ...
  return (
    ...
    typeof item.answer_event_id === "string" &&
    uuid.test(item.answer_event_id) &&
    typeof item.study_session_id === "string" &&
    uuid.test(item.study_session_id) &&
    revision(item.assessment_revision, "assessment") &&
    revision(item.question_id, "question") &&
...
function cycleSummary(value: unknown): value is AssessmentCycleSummary {
  ...
  return (
    !!item &&
    typeof item.diagnostic_set_id === "string" &&
    uuid.test(item.diagnostic_set_id) &&
    revision(item.concept_id, "concept") &&
    ...
    (item.active_set_id === null ||
      (typeof item.active_set_id === "string" && uuid.test(item.active_set_id))) &&
    [
      "passed_count",
...
function assessmentSetSummary(value: unknown): value is AssessmentSetSummary {
  ...
  return (
    ...
    (item.kind === "diagnostic"
      ...
      : typeof item.diagnostic_set_id === "string" &&
        uuid.test(item.diagnostic_set_id) &&
        item.diagnostic_set_id !== item.set_id) &&
    typeof item.set_id === "string" &&
    uuid.test(item.set_id) &&
    revision(item.target_concept_id, "concept") &&
    [
      "preparing",
      "partial_ready",
...
function assessmentSet(value: unknown): value is AssessmentSetView {
  ...
  for (const point of cycle.points) {
    ...
    if (
      ...
      !["latest_answer_event_id", "latest_set_id"].every(
        (key) =>
          row[key] === null || (typeof row[key] === "string" && uuid.test(row[key] as string)),
      )
    )
      return false;
  ...
  if (
    ...
    Number(cycle.unavailable_count) < count("unavailable")
  )
    return false;
  let published = 0,
    answered = 0,
  ...
  const valid = item.items.every((value, index) => {
    ...
    if (
      ...
      !summary.assessment_revisions.includes(row.assessment.assessment_revision)
    )
      return false;
    ...
    if (row.feedback === null)
      return !row.can_submit || ["ready", "in_progress"].includes(summary.status);
    if (
...
function safeMessage(reason: ApiReasonCode): string {
  ...
  if (reason === "SOURCE_IN_USE")
    return "這份來源已被分析引用，無法刪除；可取消勾選，不加入這次更新。";
...
function identity(value: unknown): value is LearnerIdentity {
  ...
  return (
    !!item &&
    item.schema === "learner-identity/v1" &&
    typeof item.learner_id === "string" &&
    uuid.test(item.learner_id)
  );
}
...
export class StudydyApiClient {
  ...
  private async request(
    ...
    const read = async () => {
      ...
      if (!apiError(value)) {
        ...
        throw new ApiClientError("schema", "伺服器回應格式無法辨識。", {
          status: response.status,
      ...
      const reason = knownReasons.has(value.reason_code as KnownApiReasonCode)
  ...
  sourceCapabilities(): Promise<SourceCapabilities> {
    return this.json("/v1/source-capabilities", { method: "GET" }, capabilities);
  }
  createDraft(
    name: string,
    key: string,
    ...
    return this.post(
      ...
      key,
      (value): value is { schema: "material-draft/v1"; material_id: string } => {
        const item = object(value);
        return (
          !!item &&
          item.schema === "material-draft/v1" &&
          typeof item.material_id === "string" &&
          uuid.test(item.material_id)
        );
      },
  ...
  async getMaterial(materialId: string): Promise<MaterialLibraryItem> {
    ...
    if (item.material_id !== materialId)
      throw new ApiClientError("schema", "教材身分不一致。", {
        reasonCode: "RESPONSE_SCHEMA_MISMATCH",
      });
    return item;
  }
  ...
  async discardMaterial(materialId: string): Promise<MaterialDiscardView> {
    ...
    if (view.material_id !== materialId)
      throw new ApiClientError("schema", "教材身分不一致。", {
        reasonCode: "RESPONSE_SCHEMA_MISMATCH",
      });
    return view;
  ...
  async getKnowledgeStructure(request: KnowledgeStructureRequest): Promise<KnowledgeStructureView> {
    ...
    if (view.knowledge_structure_revision !== request.structureRevision)
      throw new ApiClientError("schema", "教材結構版本不一致。", {
        reasonCode: "RESPONSE_SCHEMA_MISMATCH",
      });
    return view;
  }
  ...
  async readProgress(id: string, structureRevision: string): Promise<LearnerProgressView> {
    ...
    if (value.study_session_id !== id || value.knowledge_structure_revision !== structureRevision) {
      throw new ApiClientError("schema", "學習進度與教材版本不一致。", {
        reasonCode: "RESPONSE_SCHEMA_MISMATCH",
      });
    }
    return value;
  ...
  async applyGuidance(id: string, body: GuidanceApply): Promise<LearnerProgressView> {
    ...
    if (value.study_session_id !== id)
      throw new ApiClientError("schema", "學習進度身分不一致。", {
        reasonCode: "RESPONSE_SCHEMA_MISMATCH",
      });
    return value;
  }
  ...
  async resumeStudy(request: {
    ...
    if (
      ...
      throw new ApiClientError("schema", "學習紀錄與教材版本不一致。", {
        reasonCode: "RESPONSE_SCHEMA_MISMATCH",
      });
    }
    return restored;
  ...
  async readAssessmentPlan(id: string, conceptId: string): Promise<AssessmentPlanView> {
    ...
    if (value.study_session_id !== id || value.concept_id !== conceptId)
      throw new ApiClientError("schema", "題組範圍不一致。", {
        reasonCode: "RESPONSE_SCHEMA_MISMATCH",
      });
    return value;
  }
