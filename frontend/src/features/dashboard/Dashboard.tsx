...
export function Dashboard({ apiClient }: { apiClient: StudydyApiClient }) {
  ...
  const stats: { title: string; value: number | undefined; note: string; icon: IconName }[] = [
    { title: "教材", value: materials?.length, note: "已保存的教材", icon: "book" },
    { title: "知識地圖", value: materials?.filter(item => item.available_structures.length > 0).length, note: "可開啟地圖的教材", icon: "map" },
    { title: "已開始學習", value: materials ? studies.length : undefined, note: "有學習紀錄的教材", icon: "learning" },
    { title: "已完成", value: materials ? studies.filter(item => item.session.status === "completed").length : undefined, note: "已完成學習的教材", icon: "check" },
  ];
    ...
    <div className="dashboard-content">
      <div className="dashboard-primary">
        ...
        <section className="dashboard-overview" aria-label="學習總覽"><h2>學習總覽</h2><div className="dashboard-stats" aria-busy={materials === null && !error}>
          {stats.map((stat, index) => <article className={`dashboard-stat accent-${index}`} key={stat.title}>
            <span className="stat-icon"><Icon name={stat.icon} size={23} /></span><span className="stat-copy"><span>{stat.title}</span><strong>{error ? "—" : stat.value ?? "—"}</strong><small>{error ? "暫時無法讀取" : materials ? stat.note : "正在讀取…"}</small></span>
          </article>)}
        </div></section>
