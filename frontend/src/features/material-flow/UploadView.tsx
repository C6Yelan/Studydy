...
export function UploadView({ apiClient }: { apiClient: StudydyApiClient }) {
  ...
  const hasNonPdf = queue.some(({ file }) => formats.some(format => format.extension !== ".pdf" && file.name.toLowerCase().endsWith(format.extension)));
  return <section className="upload-page task-page" aria-labelledby="upload-title">
    ...
    <div className="upload-layout"><section className="surface upload-card" aria-label="上傳教材檔案">
      <label className={`file-drop${queue.length ? " has-file" : ""}${busy || !formatsReady ? " is-disabled" : ""}${isDragging ? " is-dragging" : ""}`}
        ...
        onDrop={event => {
          ...
          if (folder || !event.dataTransfer.types.includes("Files")) setSelectionError(folder ? "不接受資料夾，請選擇教材檔案。" : "請拖曳教材檔案，不接受文字或網址。");
          else chooseFiles(event.dataTransfer.files);
        }}>
        <input ref={fileInput} type="file" multiple accept={formats.map(format => format.extension).join(",")} aria-label="選擇教材檔案" aria-describedby={selectionError ? "upload-selection-error" : undefined} disabled={busy || !formatsReady}
        ...
        <strong>{queue.length ? "拖放或點擊以加入更多教材" : "將教材拖放到此處，或點擊選取"}</strong>
        <span>{formats.map(format => format.extension.slice(1).toUpperCase()).join("、")} · 可選多份 · 每份最大 100 MiB</span>
      </label>
      {selectionError && <p id="upload-selection-error" className="form-error" role="alert">{selectionError}</p>}
      {queue.map(item => {
      ...
      {queue.length > 0 && <p className="conversion-note">共 {queue.length} 份 · {formatFileSize(queue.reduce((total, item) => total + item.file.size, 0))}</p>}
      {capabilityError && <p className="conversion-note" role="status">{capabilityError}</p>}
      {hasNonPdf && <p className="conversion-note">非 PDF 教材會先轉換為 PDF，請在下一步確認轉換內容。</p>}
      {error && <p className="form-error" role="alert">{error}</p>}
      <button className="primary-button full-button" disabled={!queue.length || busy} onClick={() => void submit()}><Icon name="upload" size={18} />{busy ? "正在上傳…" : queue.every(item => item.status === "uploaded") && queue.length ? "繼續確認來源" : queue.some(item => item.status === "failed") ? "重試未完成的上傳" : "上傳並確認來源"}</button>
    </section><aside className="upload-aside" aria-label="教材處理說明"><section className="surface guide-card">
      <h2>Studydy 如何處理教材</h2><ol>
        <li><span>1</span><div><strong>準備教材</strong><p>各份來源分開保留；非 PDF 教材會先轉換為可分析的 PDF。</p></div></li>
        <li><span>2</span><div><strong>整理內容與來源</strong><p>讀取教材內容，需要時辨識圖片中的文字，並保留可回查的原始來源。</p></div></li>
        <li><span>3</span><div><strong>建立知識結構</strong><p>從教材整理重要概念、概念關係與建議學習順序。</p></div></li>
        <li><span>4</span><div><strong>建立知識地圖</strong><p>將整理結果呈現在知識地圖中，之後可探索概念並回查教材來源。</p></div></li>
      </ol>
    </section></aside></div>
  </section>;
}
