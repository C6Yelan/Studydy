...
export function materialDeleteCopy(material?: MaterialLibraryItem | null, sources?: SourceView[]) {
  ...
  const processing = [material?.latest_attempt, ...knownSources].some(value => value?.status === "pending" || value?.status === "running");
  const notice = processing ? `目前的教材${hasMap ? "更新" : "處理"}會先停止，再刪除這份教材。` : undefined;
  ...
  if (hasHistory) {
    ...
    const converted = knownSources.some(source => source.media_type !== "application/pdf" && source.normalized_artifact_id);
