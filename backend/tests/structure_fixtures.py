...
def build_knowledge_structure(context, state, **kwargs):
    ...
    bundle = {'schema': 'bundle-manifest/v1', 'source_set_digest': digest,
              'processing_policy': 'source-boundary-incremental/v1',
              'source_names': ['Synthetic.pdf'],
              'pages': [{'page': i, 'source_id': source_id, 'normalized_page': i}
                        for i in range(1, draft['page_count'] + 1)]}
    return finalize_knowledge_structure(draft, {
        'schema': 'structure-input-binding/v1', 'source_set_id': source_id,
        'source_set_digest': digest, 'bundle_manifest_sha256': canonical_sha256(bundle),
        'manifest': manifest, 'bundle': bundle, 'base_revision': None,
    })
