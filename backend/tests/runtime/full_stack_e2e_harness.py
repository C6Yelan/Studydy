...
from runtime.api import ApiSettings, create_app  # noqa: E402
...
class FullStackHarness:
    ...
    def start(self) -> None:
        ...
        if run_migrations(self.database_dsn, migrations_dir=DEFAULT_MIGRATIONS_DIR) != (1, 2, 3, 4):
    ...
    def _start_backend(self) -> None:
        ...
        provider_endpoint = os.environ.get("STUDYDY_E2E_PROVIDER_ENDPOINT")
        if provider_endpoint is not None:
            environment["STUDYDY_E2E_PROVIDER_ENDPOINT"] = provider_endpoint
        ...
        environment["STUDYDY_E2E_CACHE_ROOT"] = str(self.runtime_root / "cache")
...
def _backend_child() -> int:
    ...
    cache_root = os.environ.get("STUDYDY_E2E_CACHE_ROOT")
    if not database_dsn or not artifact_root or not cache_root:
        return 2
    Path(artifact_root).mkdir(mode=0o700, parents=True, exist_ok=True)
    Path(cache_root).mkdir(mode=0o700, parents=True, exist_ok=True)
    ...
    app = create_app(settings, lambda _subject: ())
    import uvicorn
