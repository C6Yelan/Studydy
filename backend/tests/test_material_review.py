...
def view():
    ...
    texts=['角色分類包括角色甲與角色乙。','角色甲負責提供資訊。','角色乙負責對外聯絡。','授課教師：某老師。','效率 Efficiency 使用8個單位的資源。']
    concepts=[]
    for i,(label,text) in enumerate(zip(labels,texts)):
        evidence={'evidence_id':f'e{i}','source_id':'source','source_name':'synthetic.pdf',
                  'normalized_page':1,'page':1,'block_order':i,'kind':'paragraph','quote':text}
    ...
    return {'schema':'knowledge-structure-view/v3','knowledge_structure_revision':'original',
            ...
            'relations':[{'relation_id':'r0','source_concept_id':'c1','target_concept_id':'c0',
                         ...
                         {'relation_id':'r1','source_concept_id':'c2','target_concept_id':'c0',
                          'type':'part_of','learner_reason':'乙是分類之一。','evidence_refs':['e0','e2']}],
...
def proposal():
    return {'assignments':[{'concept':i,'action':'keep','target':None,'issue':'none','reason':'來源有定義。','evidence':[i]} for i in range(5)],
            'alias_edits':[],'claim_edits':[],'relation_edits':[]}


def test_complete_unit_grouping_preserves_individual_points_sources_and_original():
    original=view();before=deepcopy(original);unit=prepare(original);response=proposal()
    for i in [1,2]:response['assignments'][i].update(action='group',target=0,issue='fragment',evidence=[0,i])
    ...
    assert result['preserved_claim_ids']==[f'q{i}' for i in range(5)]
    assert result['preserved_evidence_ids']==[f'e{i}' for i in range(5)]
...
def test_examples_remain_attached_and_metadata_does_not_remove_source_content():
    response=proposal();response['assignments'][1].update(action='example',target=0,issue='case',evidence=[0,1])
    ...
    assert set(result['preserved_evidence_ids'])=={f'e{i}' for i in range(5)}


def test_section_topic_cannot_be_discarded_as_cover_noise():
    response=proposal();response['assignments'][0].update(action='metadata',issue='layout')
    result=project_review(view(),prepare(),response)
...
@pytest.mark.parametrize('source_text, proposed', [('效率↑', '效率是分析因素。'), ('議價能力↓', '議價能力↑'), ('條件≤8', '條件≥8')])
def test_claim_correction_preserves_direction_and_inequality(source_text, proposed):
    from knowledge_map.material_review import _corrected_claim, ClaimEdit
    edit = ClaimEdit(claim=0, evidence=[0], reason='來源補正', meaning=proposed)
    assert _corrected_claim(edit, [{'evidence_id':'e', 'kind':'paragraph', 'quote':source_text}]) is None
...
def test_invalid_scope_or_unsafe_grouping_is_rejected(bad):
    response=proposal()
    if bad=='missing':response['assignments'].pop()
    elif bad=='duplicate':response['assignments'][-1]=deepcopy(response['assignments'][0])
    elif bad=='unknown_evidence':response['assignments'][0]['evidence']=[99]
    elif bad=='self':response['assignments'][0].update(action='group',target=0)
    else:
    ...
    with pytest.raises(ReviewError):validate_proposal(prepare(),response)


def test_parent_source_binding_is_derived_without_inventing_model_citations():
    response=proposal();response['assignments'][1].update(action='group',target=0,issue='fragment')
    result=project_review(view(),prepare(),response)
    binding=next(a for a in result['assignments'] if a['concept_id']=='c1')
    assert binding['model_evidence_ids']==['e1']
    assert binding['source_evidence_ids']==['e1'] and binding['target_evidence_ids']==['e0']
...
def test_nested_parent_is_preserved_instead_of_flattening_the_whole_lesson():
    ...
    result=project_review(view(),prepare(),response)
    assert any(u['concept_id']=='c1' and u['member_concept_ids']==['c1','c2'] for u in result['learning_units'])
...
def test_internalizing_an_example_does_not_hide_an_existing_wrong_direction():
    document=view();document['relations'][0]['type']='example'
    response=proposal();response['assignments'][1].update(action='example',target=0,issue='case')
    result=project_review(document,prepare(document),response)
    assert any(f.get('relation_id')=='r0' for f in result['findings'])


def test_alias_removal_only_changes_projection_and_cannot_invent_aliases():
    response=proposal();response['alias_edits']=[{'concept':4,'remove':['Effectiveness'],'reason':'來源為Efficiency。','evidence':[4]}]
    document=view();result=project_review(document,prepare(document),response)
    ...
    with pytest.raises(ReviewError,match='ALIAS'):project_review(document,prepare(document),response)


def test_claim_correction_cannot_change_source_number():
    response=proposal();response['claim_edits']=[{'claim':4,'meaning':'效率 Efficiency 使用9個單位的資源。','reason':'更正數量。','evidence':[4]}]
    result=project_review(view(),prepare(),response)
...
def test_claim_correction_cannot_introduce_an_uncited_other_concept():
    response=proposal();response['claim_edits']=[{'claim':1,'meaning':'角色甲也是角色乙。','reason':'補足關係。','evidence':[1]}]
    result=project_review(view(),prepare(),response)
    assert result['claim_changes']==[]
...
def test_claim_can_name_its_existing_owner_without_repeating_the_heading_citation():
    document=view();q=document['concepts'][4]['claims'][0]
    q['text']='每次使用8個資源單位。';q['evidence'][0]['quote']=q['text']
    response=proposal();response['claim_edits']=[{'claim':4,'meaning':'效率為每次使用8個資源單位。','reason':'補上所屬觀念名稱。','evidence':[4]}]
...
def test_prose_gloss_correction_does_not_weaken_code_or_identifier_protection(source,meaning,kind,allowed):
    document=view();q=document['concepts'][4]['claims'][0];q['text']=source;q['evidence'][0].update(quote=source,kind=kind)
    response=proposal();response['claim_edits']=[{'claim':4,'meaning':meaning,'reason':'整理語句。','evidence':[4]}]
    result=project_review(document,prepare(document),response)
...
def test_relation_reverse_and_prerequisite_integrity():
    document=view();document['relations'][0]['type']='example'
    response=proposal();response['relation_edits']=[{'relation':0,'action':'reverse','relation_type':'example','reason':'分類指向案例。','evidence':[0,1]}]
    result=project_review(document,prepare(document),response)
    ...
    response['relation_edits']=[];response['assignments'][1].update(action='group',target=0,issue='fragment',evidence=[0,1])
    result=project_review(document,prepare(document),response)
    ...
    assert any(u['concept_id']=='c1' for u in result['learning_units'])
...
def test_response_is_bound_to_the_exact_original_view():
    document=view();unit=prepare(document);document['concepts'][0]['claims'][0]['text']='changed'
    with pytest.raises(ReviewError,match='SOURCE_CHANGED'):project_review(document,unit,proposal())


def test_blocked_prerequisite_removal_preserves_its_original_reason():
    document=view();document['relations'][0]['type']='prerequisite'
    response=proposal();response['relation_edits']=[{'relation':0,'action':'remove','relation_type':None,'reason':'刪掉它。','evidence':[0,1]}]
    result=project_review(document,prepare(document),response)
...
def test_combining_disjoint_units_remaps_handles_and_rejects_overlapping_opinions():
    ...
    def keep(unit):
        return {'assignments':[{'concept':i,'action':'keep','target':None,'issue':'none','reason':'保留。',
                ...
                'alias_edits':[],'claim_edits':[],'relation_edits':[]}
    a,b=keep(first),keep(second);a['assignments'][1].update(action='group',target=0,issue='fragment')
    b['alias_edits']=[{'concept':1,'remove':['Effectiveness'],'evidence':[1],'reason':'別名錯置。'}]
    merged,response=combine_reviews(document,[(first,a),(second,b)])
