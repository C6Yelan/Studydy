...
def _bound_input(session, owner, run):
    ...
    for item in manifest['items']:
        for number in range(1, item['page_count'] + 1):
            expected_pages.append({'page': len(expected_pages) + 1,
                                   'source_id': item['source_id'], 'normalized_page': number})
    if bundle['pages'] != expected_pages:
        raise SourceError('SOURCE_BINDING_INVALID')
    return {'schema': 'structure-input-binding/v1', 'source_set_id': str(source_set.source_set_id),
            'source_set_digest': source_set.digest, 'bundle_manifest_sha256': run.bundle_manifest_sha256,
            'manifest': manifest, 'bundle': bundle, 'base_revision': run.base_revision}
    ...
    for item in binding['manifest']['items']:
        for prefix in ('original', 'normalized', 'mapping'):
            with open_verified_artifact(owner, UUID(item[prefix + '_artifact_id']), dsn=dsn) as blob:
                if blob.sha256 != item[prefix + '_sha256']:
                    raise SourceError('SOURCE_BINDING_INVALID')
...
def verify_structure_input(session, run, document):
    ...
    if (document.get('input_binding') != expected
        or not lock_matches_binding(run.runtime_lock_document, runtime)
        or any(runtime[k] != document['provenance'][k]
