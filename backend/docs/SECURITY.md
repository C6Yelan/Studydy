...
本文件說明 Studydy 後端在「身分驗證 / 註冊 / 忘記密碼 / JWT」相關的安全設計原則與最低要求。內容以專題階段可落地的做法為主，並標記後續可強化項目。
...
- Secrets（JWT secret、DB credentials 等）

### 目標
- 避免帳號枚舉（Account Enumeration）
- 降低暴力破解、憑證填充（Brute force / Credential stuffing）風險
- 避免弱密鑰與 JWT 常見誤用（alg none、未驗 exp、弱 secret）
...
- 若未來導入 session 管理，重設後可選擇失效所有舊 session/token

---

## 4) JWT 安全要求（Access Token）

### 4.1 JWT 驗證最低要求
- 必須驗證：
  - 簽章有效（signature valid）
  - 演算法符合預期（只接受允許的 alg；禁止 `alg: none` 類型）
  - `exp`（過期時間）確實有被檢查（過期即拒絕）

### 4.2 JWT Secret（HMAC）強度與管理
若使用對稱式 HMAC（例如 HS256）：
- secret 必須足夠長且不可猜（避免被暴力破解）
- 不得重用在不同環境（dev/staging/prod 應分離）
- 必須可輪替（rotation）

### 4.3 Token 使用建議
- access token 有效期應偏短（專題階段至少要有 `exp`）
- 只透過 HTTPS 傳輸
- 禁止把 token、驗證碼、密碼寫入 log

---

## 5) Secrets 管理（JWT secret / DB credentials / API keys）

最低要求：
- **禁止**把 secrets 寫死在程式碼或提交到 Git（包含 `.env`、連線字串、JWT secret、私鑰/憑證）
...
- OWASP WSTG：Testing JSON Web Tokens（強密鑰、驗證 alg/exp、token 安全儲存與傳輸）
