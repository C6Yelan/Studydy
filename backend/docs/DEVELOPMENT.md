...
- Core 層（`app/core/`）：集中「可配置常數、env 讀取、安全工具（hash）」。
...
- Session/CORS（環境變數）：`SESSION_SECRET_KEY`、`CORS_ORIGINS`（於 `app/main.py` 讀取）
...
* PR 前檢查：
  ...
  1. `pytest` 全綠
  2. Swagger 可開啟（`/docs`）
  3. 禁止提交 `.env`/secrets（含 session secret、DB 連線字串、私鑰/憑證）
