...
ALTER TABLE assessments ADD CONSTRAINT assessment_target_scope
 UNIQUE (assessment_revision, study_session_id, knowledge_structure_revision, target_concept_id, target_claim_id);
...
CREATE TABLE assessment_sets (
 ...
 status text NOT NULL CHECK (status IN ('preparing','partial_ready','failed','ready','in_progress','completed','cancelled')),
 ...
 FOREIGN KEY (learner_id,material_id,study_session_id,knowledge_structure_revision)
  REFERENCES study_sessions(learner_id,material_id,study_session_id,knowledge_structure_revision),
...
CREATE UNIQUE INDEX one_active_assessment_set ON assessment_sets(study_session_id)
...
CREATE TABLE assessment_set_items (
 ...
 state text NOT NULL CHECK (state IN ('pending','generating','verified','published','failed','omitted')),
 ...
 FOREIGN KEY (assessment_revision,study_session_id,knowledge_structure_revision,target_concept_id,target_claim_id)
  REFERENCES assessments(assessment_revision,study_session_id,knowledge_structure_revision,target_concept_id,target_claim_id),
...
END;
$$;
CREATE TRIGGER assessment_set_item_immutable BEFORE UPDATE ON assessment_set_items
...
BEGIN
 IF OLD.sealed_at IS NOT NULL AND
  ROW(NEW.learner_id,NEW.material_id,NEW.study_session_id,NEW.knowledge_structure_revision,NEW.target_concept_id,
      NEW.target_plan,NEW.requested_count,NEW.runtime_lock_document,NEW.execution_identity,NEW.sealed_at)
  IS DISTINCT FROM
  ROW(OLD.learner_id,OLD.material_id,OLD.study_session_id,OLD.knowledge_structure_revision,OLD.target_concept_id,
      OLD.target_plan,OLD.requested_count,OLD.runtime_lock_document,OLD.execution_identity,OLD.sealed_at) THEN
  RAISE EXCEPTION 'sealed assessment set scope is immutable';
 END IF;
 RETURN NEW;
END;
$$;
CREATE TRIGGER assessment_set_scope_immutable BEFORE UPDATE ON assessment_sets
